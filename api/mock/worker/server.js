...
// 路由复用 routes/worker.js,静态托管 docs/worker;启动: node api/mock/worker/server.js
// 默认 0.0.0.0:8091,MOCK_WORKER_PORT 覆盖;跨域放行供 file:// 页面调试。
'use strict';
...
const server = http.createServer(async (req, res) => {
  ...
  if (u.pathname.startsWith('/api/worker/v1')) {
    ...
    const result = workerRoute.route(method, apiPath, u.searchParams, body);
