// 师傅端路由 —— 按 api/openapi/worker.yaml 的方法/路径返回假数据。
// 输入 pathname 已去 /api/worker/v1 前缀;query 为 URLSearchParams,body 为已解析 JSON。
'use strict';

const data = require('../worker/data.js');
...
function ticketsFiltered(status) {
...
function route(method, pathname, query, body) {
  ...
  if (method === 'GET') {
    ...
    if (p === 'tickets') return ticketsFiltered(query ? query.get('status') || 'all' : 'all');
  ...
  if (method === 'POST') {
    ...
    if (p === 'tickets/' + ticketNo + '/scan-bind') return scanBindResult(body);
