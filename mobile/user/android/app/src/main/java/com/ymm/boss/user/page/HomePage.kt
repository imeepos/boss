...
@Composable private fun Header(data: HomeData, nav: Nav) {
    Column(Modifier.fillMaxWidth().height(300.dp).background(Brush.linearGradient(listOf(Palette.primary, Palette.primary2))).padding(horizontal = 24.dp, vertical = 20.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { Column(Modifier.weight(1f)) { Text("早上好，${data.customerName}", color = Color.White, fontSize = 25.sp, fontWeight = FontWeight.Bold); Text(data.phoneMasked, color = Color.White, fontSize = 16.sp, modifier = Modifier.padding(top = 9.dp)) }; Box(Modifier.size(42.dp).clickable { nav.push(Route.Messages) }) { Icon(Icons.Outlined.List, "消息", tint = Color.White, modifier = Modifier.fillMaxSize().padding(3.dp)); if (data.hasUnread) Box(Modifier.size(11.dp).background(Palette.err, CircleShape).align(Alignment.TopEnd)) } }
...
@Composable private fun QuickGrid(nav: Nav) {
    val entries = listOf(Quick("办套餐", Icons.Outlined.ShoppingCart, Palette.primary, Route.Products), Quick("查订单", Icons.Outlined.List, Palette.success, Route.Orders), Quick("缴费用", Icons.Outlined.ShoppingCart, Palette.orange, Route.Pay), Quick("报故障", Icons.Outlined.Person, Palette.purple, Route.Fault), Quick("充值", Icons.Outlined.ShoppingCart, Palette.primary, Route.Topup), Quick("查用量", Icons.Outlined.List, Palette.success, Route.Usage), Quick("消息", Icons.Outlined.List, Palette.orange, Route.Messages), Quick("客服", Icons.Outlined.Person, Palette.purple, Route.Service))
