    ...
    Column(
        ...
        Column(
            ...
            Column(
                ...
                Text(stringResource(R.string.onboard_title), fontSize = 20.sp, fontWeight = FontWeight.Bold, color = Ink)
                Text(stringResource(R.string.onboard_subtitle), fontSize = 12.sp, color = Muted,
                    modifier = Modifier.padding(top = 6.dp, bottom = 12.dp))

                AuthInputRow(name, { name = it }, stringResource(R.string.auth_hint_name), Icons.Outlined.Person, KeyboardType.Text)
            ...
            // 提交按钮
            AuthPrimaryButton(stringResource(R.string.onboard_submit), enabled = agreed && !busy, loading = busy,
                onClick = {
                    busy = true
            ...
            Row(
                ...
                Text(stringResource(R.string.auth_has_account), fontSize = 12.sp, color = Muted)
                Text(stringResource(R.string.auth_go_login), fontSize = 12.sp, color = Primary, fontWeight = FontWeight.W500,
...
private fun OnboardHeader(onBack: () -> Unit) {
    Box(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(stringResource(R.string.onboard_title), color = Color.White, fontSize = 16.sp, fontWeight = FontWeight.SemiBold,
...
private fun OnboardSuccess(onBack: () -> Unit) {
    Column(
        ...
        Text(stringResource(R.string.onboard_submitted), fontSize = 20.sp, fontWeight = FontWeight.Bold, color = Ink)
        Text(stringResource(R.string.onboard_submitted_desc), fontSize = 14.sp, color = Muted,
            modifier = Modifier.padding(top = 8.dp))
        Text(stringResource(R.string.onboard_submitted_note), fontSize = 12.sp, color = Muted,
            modifier = Modifier.padding(top = 4.dp))

        Spacer(Modifier.height(32.dp))

        AuthPrimaryButton(stringResource(R.string.onboard_back_login), enabled = true, onClick = onBack)
    }
}
...
private fun doOnboard(
    ...
    if (name.isBlank() || phone.isBlank() || code.isBlank() || idCardNo.isBlank()) {
        onDone(false, s(R.string.err_form_incomplete)); return
    }
    ...
    scope.launch {
        try {
            ...
            onDone(false, s(R.string.err_submit_failed, friendlyMessage(e)))
        }
    }
}
