...
fun ServiceScreen(nav: NavHost) {
    val state by loadOnce { MiscApi.serviceMessages() }
    val ctx = LocalContext.current

    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
        TopBar("联系调度", onBack = { nav.pop() }, action = "转人工", onAction = { toast(ctx, "已转人工。") })
        ...
        Row(Modifier.fillMaxWidth().padding(12.dp),
            ...
            PrimaryButton("转单/改派", modifier = Modifier.weight(1f)) { nav.push(Screen.Transfer(FALLBACK_NO)) }
        }
