...
- **行为缺口｜置备资产不回填 tag 双向绑定**:`POST /provision/assets`(internal/domain/asset/pg_write.go::CreateAsset)只写 `assets.tag_id`,不回填 `tags.bound_asset_id`/`tags.status`。而环节9 扫码核对(internal/domain/quadlink/pg_scan.go::VerifyScan)要求 `tags.bound_asset_id` 非空且等于 quad_link.asset_id,否则报 40920"扫码与预绑定不一致"。e2e 测试(internal/app/e2e_pg_integration_test.go:224)是建 tag 时手工传 `BoundAssetID + Status="BOUND"` 才绕过。→ 置备端点或 CreateAsset 应在 tag_id 非空时同步 `UPDATE tags SET bound_asset_id=$asset, status='BOUND'`,与 e2e 口径对齐。
