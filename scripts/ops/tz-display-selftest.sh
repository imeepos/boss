...
if (cd "$ADMIN" && pnpm vitest run src/lib/format.test.ts > /tmp/tz-display-format-test.log 2>&1); then
...
if (cd "$ADMIN" && pnpm typecheck > /tmp/tz-display-typecheck.log 2>&1); then echo "PASS admin typecheck"; else cat /tmp/tz-display-typecheck.log; fail_step "admin typecheck"; fi
