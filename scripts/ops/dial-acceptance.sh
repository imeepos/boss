...
one_run() {
  ...
  FAIL_REASON=""
  # 自举资源:地址 + 分光器 + 2端口 + 标签/资产(与 mainchain-acceptance.sh 同一 acc_ 标记体系)
  ...
  res=$(api POST /provision/resources "{\"code\":\"SPL-ACC-$suffix\",\"name\":\"拨号验收分光器$run\",\"type\":\"SPLITTER\",\"addressId\":$addr_id,\"legalEntityId\":1}") || return 1
