...
# 断言: P0 造数(acc_w1 前缀,施工类供应商/项目/PLANNED 设施) / P1 建施工类供应商
#   / P2 建项目挂承包商 / P3 录清单(数量x单价,金额后端计算) / P4 开工+竣工 ACCEPTED
#   / P5 发起结算(应付=SUM(amount),curl 断言金额与状态) / P6 确认结算 SETTLED
#   / P7 作废 SETTLED→VOIDED+重开新单 / P8 负例(未指定承包商拒结算,ACCEPTED 后改定额拒)
#   / RES 自清理残留为零+孤儿巡检。
# 信号: PASS/FAIL 逐条;收尾 [e2e-w1-settlement RESULT: ...] 可 grep。
...
DELETE FROM construction_items WHERE facility_code='$FAC' OR project_id IN (SELECT id FROM construction_projects WHERE proj_no LIKE '${TAG}-%');
...
UNION ALL SELECT 'items_orphan=' || count(*) FROM construction_items WHERE facility_code='$FAC' AND project_id NOT IN (SELECT id FROM construction_projects);
SQL
}
...
# P0 前置:CLS00098 PLANNED 设施(建单挂明细用;沿用既有 e2e 设施,不动归属)。
HAS=$(sqlval "SELECT to_regclass('public.construction_projects') IS NOT NULL AND to_regclass('public.construction_settlements') IS NOT NULL;")
if [ "$HAS" != "t" ]; then echo "FAIL: [P0] 施工/结算表未就绪(CI 未部署迁移 000205/000206),等待部署窗口" >&2; echo "[e2e-w1-settlement RESULT: FAIL wait-deploy]"; exit 2; fi
sqlval "INSERT INTO odn_facility(code, kind, prv_code, city_prefix, grid_code, name, status, lifecycle_status) VALUES ('$FAC','CLS','PHL001','MNL',1,'$TAG 设施','IN_USE','PLANNED') ON CONFLICT (code) DO UPDATE SET lifecycle_status='PLANNED' RETURNING code;" >/dev/null
LC=$(sqlval "SELECT lifecycle_status FROM odn_facility WHERE code='$FAC';")
assert_eq P0 "$LC" "PLANNED" "设施 $FAC 备妥"

# P1 建施工类供应商(contractorType=CONSTRUCTION + 资质)。
...
assert_eq P1 "200" "$HTTP_CODE" "建施工类供应商"
SUP_ID=$(jf "$BODY" "d['data']['id']")
if [ -z "$SUP_ID" ] || [ "$SUP_ID" = "None" ]; then bad P1 "供应商 id 缺失 body=$BODY"; fi
req GET "/procurement/suppliers"
CT=$(jf "$BODY" "[s for s in d['data'] if s['id']==$SUP_ID][0]['contractorType']")
assert_eq P1 "CONSTRUCTION" "$CT" "供应商承建类型回读"

# P2 建项目并挂承包商。
req POST /odn/constructions "{\"projNo\":\"$PROJ\",\"name\":\"$TAG 施工单\",\"prvCode\":\"PHL001\",\"cityPrefix\":\"MNL\"}"
assert_eq P2 "200" "$HTTP_CODE" "建施工项目"
PID=$(jf "$BODY" "d['data']['projNo']" >/dev/null; sqlval "SELECT id FROM construction_projects WHERE proj_no='$PROJ';")
req PUT "/odn/constructions/$PID/contractor" "{\"contractorId\":$SUP_ID}"
assert_eq P2 "200" "$HTTP_CODE" "指定承包商"
req GET "/odn/constructions/$PID"
CN=$(jf "$BODY" "d['data']['project']['contractorName']")
assert_eq P2 "$TAG 施工承包商" "$CN" "承包商名称快照"

# P3 录清单两行:数量x单价,金额后端计算(2x350=700;1.5x80=120,合计 820)。
req POST "/odn/constructions/$PID/items" "{\"facilityCode\":\"$FAC\",\"quantity\":2,\"unitPrice\":350}"
assert_eq P3 "200" "$HTTP_CODE" "追加明细1"
req POST "/odn/constructions/$PID/items" "{\"facilityCode\":\"$FAC\",\"quantity\":1.5,\"unitPrice\":80}"
# 同设施唯一约束 → 第二行应被拒(40900);改用另一设施补第二行。
sqlval "INSERT INTO odn_facility(code, kind, prv_code, city_prefix, grid_code, name, status, lifecycle_status) VALUES ('${TAG}F2','CLS','PHL001','MNL',1,'$TAG 设施2','IN_USE','PLANNED') ON CONFLICT (code) DO UPDATE SET lifecycle_status='PLANNED';" >/dev/null
req POST "/odn/constructions/$PID/items" "{\"facilityCode\":\"${TAG}F2\",\"quantity\":1.5,\"unitPrice\":80}"
assert_eq P3 "200" "$HTTP_CODE" "追加明细2"
...
assert_eq P4 "200" "$HTTP_CODE" "开工"
req POST "/odn/constructions/$PID/accept" "{\"note\":\"$TAG as-built\"}"
assert_eq P4 "200" "$HTTP_CODE" "竣工"
req GET "/odn/constructions/$PID"
ST=$(jf "$BODY" "d['data']['project']['status']")
assert_eq P4 "ACCEPTED" "$ST" "项目状态"
...
assert_eq P6 "200" "$HTTP_CODE" "发起结算"
TOTAL=$(jf "$BODY" "d['data']['totalAmount']")
assert_eq P6 "820" "$TOTAL" "应付金额=清单汇总"
SSTAT=$(jf "$BODY" "d['data']['status']")
assert_eq P6 "PENDING" "$SSTAT" "结算单状态"
SNO=$(jf "$BODY" "d['data']['settlementNo']")
...
assert_eq P7 "200" "$HTTP_CODE" "确认结算"
...
assert_eq P7 "200" "$HTTP_CODE" "作废"
req POST "/odn/constructions/$PID/settlements"
assert_eq P7 "200" "$HTTP_CODE" "作废后重开(新单)"
TOTAL2=$(jf "$BODY" "d['data']['totalAmount']")
assert_eq P7 "820" "$TOTAL2" "新单应付金额不变"
# 作废缺原因被拒。
...
ORPHAN=$(sqlval "SELECT count(*) FROM construction_items WHERE facility_code IN ('$FAC','${TAG}F2') OR project_id IN (SELECT id FROM construction_projects WHERE proj_no LIKE '${TAG}-%');")
assert_eq RES "0" "$ORPHAN" "孤儿巡检(结算单/项目/明细/供应商归零)"
