#!/usr/bin/env bash
# 构建并通过 ADB 安装 worker (师傅端) Android 包。
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ANDROID_DIR="$ROOT_DIR/mobile/worker/android"
PACKAGE_NAME="com.ymm.boss.worker"
BUILD_TYPE="debug"
DEVICE=""
BASE_URL=""
...
usage() {
  cat <<'EOF'
用法: scripts/build-install-worker-android.sh [选项]
...
示例:
  scripts/build-install-worker-android.sh
  scripts/build-install-worker-android.sh --device EAORGQYL45XGJJCY
  scripts/build-install-worker-android.sh --release --no-install
EOF
}
