...
const GROUPS = [
  ...
  ['订单·派工·计费·税务', '#ffe6cc', [
    ['orders', '订单(12环节)', 'PK id', 'UQ order_no', 'sFK customer_id/offer_id/address_id', 'sFK channel_id', 'legal_entity快照+region_path'],
  ...
  ['资产·四码', '#e1d5e7', [
    ...
    ['quad_links', '四码合一', 'PK id', 'sFK asset/customer/port/address 各UQ', 'legal_entity快照'],
  ]],
  ['网络资源·监控·开通', '#f8cecc', [
  ...
  ['师傅域', '#fff2cc', [
    ...
    ['worker_registrations', '师傅注册申请', 'FK group_id', 'sFK worker_id(通过后回填)'],
    ['worker_real_name_verifications', '师傅实名', 'FK worker_id'],
...
const EDGES = [
  ...
  ['real_name_verifications', 'customers', 'N:1', 'fk'],
  ['customer_real_name_verifications', 'customers', 'N:1', 'fk'],
  ...
  ['bills', 'customers', 'N:1', 'fk'], ['payments', 'bills', 'N:1', 'fk'],
  ...
  ['worker_real_name_verifications', 'workers', 'N:1', 'fk'],
