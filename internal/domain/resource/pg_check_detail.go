...
func (s *PGStore) CheckDetail(ctx context.Context, addressID int64) (*CheckDetail, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("resource: check detail query: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("resource: check detail rows: %w", err)
	}
	codes, err := s.db.Query(ctx, `SELECT port_code FROM ports WHERE address_id = $1 AND status = 'IDLE' ORDER BY id`, addressID)
	if err != nil {
		return nil, fmt.Errorf("resource: check detail idle ports: %w", err)
	}
	defer codes.Close()
	for codes.Next() {
		var code string
		if err := codes.Scan(&code); err != nil {
			return nil, fmt.Errorf("resource: check detail idle scan: %w", err)
		}
		d.IdleCodes = append(d.IdleCodes, code)
	}
	return d, codes.Err()
}
