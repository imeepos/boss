...
func (s *PGStore) AppendTaxEvent(ctx context.Context, e TaxEvent) (int64, error) {
	...
	err := s.db.QueryRow(ctx, `
		INSERT INTO invoice_tax_events(invoice_id, event, tax_status_after, tax_no, fail_reason, external_id, operator_account_id)
		VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
		e.InvoiceID, e.Event, e.TaxStatusAfter, e.TaxNo, e.FailReason, e.ExternalID, e.OperatorAccountID).Scan(&id)
	if err != nil {
...
func (s *PGStore) ListTaxEvents(ctx context.Context, invoiceID int64) ([]TaxEvent, error) {
	rows, err := s.db.Query(ctx, `
		SELECT id, invoice_id, event, tax_status_after, tax_no, fail_reason, external_id, operator_account_id, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.InvoiceID, &e.Event, &e.TaxStatusAfter, &e.TaxNo,
			&e.FailReason, &e.ExternalID, &e.OperatorAccountID, &e.CreatedAt); err != nil {
