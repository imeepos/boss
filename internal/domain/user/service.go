...
type Service interface {
	...
	// UpdateSelfProfile 自助改基本资料:仅 real_name/phone,组织归属仍走受权流程。
	UpdateSelfProfile(ctx context.Context, accountID int64, realName, phone string) error
	...
	// 祖先链与命中节点一致回填 hasChildren。hasMore=true 表示命中超出单页 20 条被截断,
	// 调用方应提示用户收紧关键字。
	...
	SetEntityStaffStatus(ctx context.Context, entityID, accountID int64, status int16) error
