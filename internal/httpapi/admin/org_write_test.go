...
func TestOrgWriteHandlers(t *testing.T) {
	...
	t.Run("建部门 无权限 403", func(t *testing.T) {
		...
		w := postBodyAuth(t, r, "/api/admin/v1/departments", `{"legalEntityId":1,"name":"财务部"}`, token)
		if w.Code != 403 {
	...
	t.Run("改部门 成功", func(t *testing.T) {
		r := newTestRouter(&fakeUser{permOk: true}, mgr)
		w := putAuth(t, r, "/api/admin/v1/departments/1", `{"legalEntityId":2,"name":"客服部"}`, token)
		if w.Code != 200 {
			t.Fatalf("status=%d body=%s", w.Code, w.Body.String())
		}
	})
	...
	t.Run("建岗位 无权限 403", func(t *testing.T) {
		r := newTestRouter(&fakeUser{permOk: false}, mgr)
		w := postBodyAuth(t, r, "/api/admin/v1/posts", `{"deptId":1,"code":"agent","name":"客服坐席"}`, token)
		if w.Code != 403 {
			t.Fatalf("status=%d want 403", w.Code)
		}
	})
	...
	t.Run("建岗位 带 roles 成功", func(t *testing.T) {
		r := newTestRouter(&fakeUser{permOk: true}, mgr)
		w := postBodyAuth(t, r, "/api/admin/v1/posts",
	...
	t.Run("改岗位 成功", func(t *testing.T) {
		r := newTestRouter(&fakeUser{permOk: true}, mgr)
		w := putAuth(t, r, "/api/admin/v1/posts/1",
	...
	t.Run("建岗位 缺参 参数错误", func(t *testing.T) {
		r := newTestRouter(&fakeUser{permOk: true}, mgr)
		w := postBodyAuth(t, r, "/api/admin/v1/posts", `{"deptId":1}`, token)
		var env struct {
			Code int `json:"code"`
		}
		_ = json.Unmarshal(w.Body.Bytes(), &env)
