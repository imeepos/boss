package adminapi

// 用户数据域缺口路由:契约 admin.yaml 汇总中尚未落地的端点(worker.yaml faqs/
// device-maintenances/hall-items/service-messages、intel.yaml POST /reports、
// sys.yaml DELETE /accounts)。接线(挂 authed 组)由父会话在 http.go 完成。
// handler 实现在 userdata_gap_handlers.go。

import (
	"github.com/gin-gonic/gin"

	"github.com/ymm-001/boss/internal/app"
	"github.com/ymm-001/boss/internal/domain/user"
	"github.com/ymm-001/boss/pkg/apitypes"
)

// registerUserdataGapRoutes 注册用户数据域缺口路由。
func registerUserdataGapRoutes(g *gin.RouterGroup, a *app.Application) {
	registerGapFaqRoutes(g, a)
	registerGapDeviceRoutes(g, a)
	registerGapMessageRoutes(g, a)
	registerGapReportRoutes(g, a)
	registerGapAccountRoutes(g, a)
}

// registerGapFaqRoutes FAQ 知识库(worker.yaml /faqs 族):复用 user_faqs 表。
func registerGapFaqRoutes(g *gin.RouterGroup, a *app.Application) {
	ud := a.UserData
	udList(g, a, "/faqs", "menu:userdata", func(c *gin.Context) ([]map[string]any, error) {
		return ud.ListUserFaqs(c.Request.Context())
	})
	g.POST("/faqs", requirePerm(a.User, "menu:userdata"), gapCreateFaq(a))
	g.PUT("/faqs/:faqId/toggle", requirePerm(a.User, "menu:userdata"), gapToggleFaq(a))
}

// registerGapDeviceRoutes 设备健康观察 + 抢单池。
func registerGapDeviceRoutes(g *gin.RouterGroup, a *app.Application) {
	g.GET("/device-maintenances", requirePerm(a.User, "menu:alarm"), gapListMaintenances(a))
	g.GET("/hall-items", requirePerm(a.User, "menu:dispatch"), gapListHallItems(a))
}

// registerGapMessageRoutes 调度会话记录(worker.yaml /service-messages):
// 复用 worker_messages 台账。
func registerGapMessageRoutes(g *gin.RouterGroup, a *app.Application) {
	g.GET("/service-messages", requirePerm(a.User, "menu:dispatch"), gapListServiceMessages(a))
	g.POST("/service-messages", requirePerm(a.User, "menu:dispatch"), gapSendServiceMessage(a))
}

// registerGapReportRoutes 生成经营分析报告(intel.yaml POST /reports)。
func registerGapReportRoutes(g *gin.RouterGroup, a *app.Application) {
	g.POST("/reports", requirePerm(a.User, "menu:report"), gapGenerateReport(a))
}

// registerGapAccountRoutes 删除账号(sys.yaml DELETE /accounts/{accountId})。
func registerGapAccountRoutes(g *gin.RouterGroup, a *app.Application) {
	g.DELETE("/accounts/:accountId", requirePerm(a.User, "menu:account"), gapDeleteAccount(a))
}

// gapDisableAccount 取原行回写停用;返回 false 表示已响应(未命中/写失败)。
func gapDisableAccount(c *gin.Context, a *app.Application, id int64) bool {
	rows, err := a.User.ListAccounts(c.Request.Context())
	if err != nil {
		respondErr(c, err)
		return false
	}
	var row *user.AccountRow
	for i := range rows {
		if rows[i].ID == id {
			row = &rows[i]
			break
		}
	}
	if row == nil {
		respond(c, apitypes.CodeNotFound, nil)
		return false
	}
	return gapWriteDisabled(c, a, row)
}

// gapWriteDisabled 按原行构造 AccountInput,仅翻转 status=0。
func gapWriteDisabled(c *gin.Context, a *app.Application, row *user.AccountRow) bool {
	off := int16(0)
	in := user.AccountInput{
		Username: row.Username, RealName: row.RealName, Phone: row.Phone,
		RoleCode: row.RoleCode, Status: &off,
	}
	if row.LegalEntityID != 0 {
		in.LegalEntityID = &row.LegalEntityID
	}
	if row.DeptID != 0 {
		in.DeptID = &row.DeptID
	}
	if row.PostID != 0 {
		in.PostID = &row.PostID
	}
	if row.RegionScope != "" {
		in.RegionScope = &row.RegionScope
	}
	if err := a.User.UpdateAccount(c.Request.Context(), row.ID, in); err != nil {
		respondErr(c, err)
		return false
	}
	return true
}

// defaultGapStr 空串兜底。
func defaultGapStr(s, def string) string {
	if s == "" {
		return def
	}
	return s
}
