...
func registerPortalAuthRoutes(pub *gin.RouterGroup, a *app.Application, mgr *auth.Manager) {
	pub.POST("/auth/login", portalLoginHandler(a, mgr))
	pub.POST("/auth/sms-code", portalIssueSmsHandler(a))
	pub.POST("/auth/register", portalRegisterHandler(a, mgr))
	pub.POST("/auth/reset-password", portalResetPasswordHandler(a))
}
