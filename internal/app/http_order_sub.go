...
func registerOrderSubRoutes(g *gin.RouterGroup, a *Application) {
	// 报障与投诉(CS 客服域):列表 + 办结。
	...
	g.POST("/complaints/:ticketNo/close", requirePerm(a.User, "menu:complaint"), func(c *gin.Context) {
		...
		a.recordAudit(c, "org.update", "complaint", c.Param("ticketNo"), map[string]any{"op": "close"})
	...
	g.POST("/activation-callbacks/:callbackId/retry", requirePerm(a.User, "menu:callback"), func(c *gin.Context) {
		...
		a.recordAudit(c, "org.update", "activation_callback", c.Param("callbackId"), map[string]any{"op": "retry"})
		respond(c, apitypes.CodeOK, gin.H{"ok": true})
	})
}
