// 供应商管理抽屉:列表 + 新增(POST /procurement/suppliers)+ 停用(POST /:id/disable,ConfirmDialog)。
// 能力对齐:后台此前无任何供应商管理入口,disable/create API 悬空(PP1-C 审计项 5)。
...
export function SuppliersDrawer({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ code: '', name: '', contactName: '', contactPhone: '', legalEntityId: 1 })
  ...
  const create = async () => {
    if (busy) return
    if (!form.name.trim() || !form.code.trim()) { toast.error(d.supErrName); return }
    setBusy(true)
    try {
      await apiFetch('/procurement/suppliers', { method: 'POST', body: form })
      toast.success(d.supCreate)
      setForm({ code: '', name: '', contactName: '', contactPhone: '', legalEntityId: 1 })
      load()
      onSaved()
    } catch (e) {
    ...
    <Drawer title={d.suppliersTitle} onClose={onClose} width={680}
      ...
      <div className="flex flex-col gap-3">
        <div className="rounded-sm border border-[var(--shell-side-border)] p-3">
          <div className="mb-2 text-[13px] font-medium text-[var(--shell-heading)]">{d.supCreate}</div>
          <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
            ...
            <Button size="sm" className="h-8 px-4 text-[13px]" disabled={busy} onClick={create}>{d.supCreate}</Button>
          </div>
        </div>
        {error ? (
