...
// 字段定义:[key, 标签, 必填, 占位提示]
const TEXT_FIELDS: [keyof CountryRow, string, boolean][] = [
  ['alpha2', 'ISO 3166-1 alpha-2', true],
  ['alpha3', 'alpha-3', true],
  ['numericCode', 'numeric (UN M49)', true],
  ['shortName', 'short name', true],
  ['fullName', 'full name', false],
  ['m49Region', 'M49 region', false],
  ['postalRegex', 'postal regex', false],
]
...
export function CountryForm({ initial, editing, onDone, onCancel }: {
  ...
  return (
    ...
    >
      <FormGrid form={form} editing={editing} onChange={setForm} />
    </Drawer>
  )
}

// FormGrid 两列表单栅格;编辑态主键 alpha2 只读。
function FormGrid({ form, editing, onChange }: {
  form: CountryRow
  editing: boolean
  ...
  return (
    <div className="geo-form">
      {TEXT_FIELDS.map(([k, label, req]) => (
        <div key={k} className={`geo-field ${k === 'shortName' || k === 'fullName' ? 'full' : ''}`}>
          <label>
            {req && <span className="req">*</span>}{label}
          </label>
          <input
            className="geo-input"
      ...
      <div className="geo-field">
        <label><span className="req">*</span>continent</label>
      ...
      <div className="geo-field">
        <label><span className="req">*</span>status</label>
