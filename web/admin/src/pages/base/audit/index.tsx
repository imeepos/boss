...
export default function AuditPage() {
  ...
  const [rawEntries, setRawEntries] = useState<Record<string, AuditEntry>>({})
  ...
  const load = () => {
    ...
    apiFetch<{ items: AuditEntry[] }>('/audit-logs')
      .then((d) => {
        const items = d?.items ?? []
        setRawEntries(Object.fromEntries(items.map((e) => [String(e.id), e])))
        setRows(items.map(toAuditLog))
      })
  ...
  const detailEntry = detail ? rawEntries[detail] : null
    ...
    <div>
      ...
      {detailRow && (
        <Drawer title={t.pages.audit.detailTitle} onClose={() => setDetail(null)}
          ...
          {detailEntry?.detail && (
            <div className="mt-2">
              <div className="mb-1 text-[13px] font-medium text-[var(--shell-group-title)]">{t.pages.audit.detailJson}</div>
              <pre className="m-0 max-h-60 overflow-auto rounded-sm border border-[var(--shell-side-border)] bg-[var(--shell-input-bg)] p-3 font-mono text-[12px] whitespace-pre-wrap break-all text-[var(--shell-content-text)]">{detailEntry.detail}</pre>
            </div>
          )}
        </Drawer>
      )}
