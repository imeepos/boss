...
type Tab = 'grids' | 'facilities' | 'sites' | 'devices' | 'coverage' | 'constructions' | 'assets'
...
type AssetReg = { registrationNo: string; assetId: number; assetCode: string; assetStatus: string }
type Facility = { code: string; kind: string; prvCode: string; cityPrefix: string; gridCode: number; name: string; lat: number | null; lng: number | null; status: string; assetReg?: AssetReg | null }
type Site = { prvCode: string; cityPrefix: string; siteNo: number; name: string; lat: number | null; lng: number | null; status: string }
type Device = { id: number; code: string; kind: string; prvCode: string; cityPrefix: string; siteNo: number; parentId: number; name: string; lat: number | null; lng: number | null; status: string; assetReg?: AssetReg | null }
...
export default function ODNPage() {
  ...
  return <div>
    ...
    {showForm && tab !== 'coverage' && tab !== 'constructions' && tab !== 'assets' && <Form tab={tab} busy={busy} submit={submit} g={g} grids={grids} devices={devices} />}
