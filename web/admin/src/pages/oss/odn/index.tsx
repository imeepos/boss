...
export default function ODNPage() {
  ...
  const paged = tab === 'grids'
    ...
    : tab === 'facilities'
      ? <FacilityTable rows={slice(visFacilities)} onRetire={(code) => retire('/odn/facilities/' + encodeURIComponent(code))} g={g} />
      : tab === 'sites'
        ? <SiteTable rows={slice(visSites)} onRetire={(n) => retire('/odn/sites/' + n)} g={g} />
        : <DeviceTable rows={slice(devices)} onRetire={(id) => retire('/odn/devices/' + id)} g={g} />
