...
export default function ClientReleasePage() {
  ...
  const [form, setForm] = useState({ app: 'user', version: '', versionCode: '', minSupportedCode: '', notes: '', file: undefined as File | undefined })
  ...
  const submitUpload = async () => {
    if (!form.file || !form.version || !form.versionCode) { setError(s.uploadFail); return }
    ...
    try {
      ...
      setForm({ ...form, version: '', versionCode: '', minSupportedCode: '', notes: '', file: undefined })
      load()
    } catch (e) { setError(e instanceof Error ? e.message : s.uploadFail) }
    setUploading(false)
  }
  ...
  return <div>
    ...
    <div className="mb-4 rounded-md border border-[var(--shell-card-border)] bg-[var(--shell-card-bg)] p-4">
      <div className="grid items-end gap-3 md:grid-cols-6">
        ...
        <button className="primary h-8 cursor-pointer rounded-sm px-4 text-[13px]" disabled={uploading} onClick={submitUpload}>{s.upload}</button>
      </div>
      <label className="mt-3 block text-xs">{s.fNotes}<input className={inputCls + ' mt-1'} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label>
    </div>
    ...
    {editing && <div className="mt-4 rounded-md border border-[var(--shell-card-border)] bg-[var(--shell-card-bg)] p-4">
      ...
      <div className="mt-3 flex gap-3">
        <button className="primary h-8 cursor-pointer rounded-sm px-4 text-[13px]" disabled={busy} onClick={savePatch}>{s.save}</button>
        <button className="h-8 cursor-pointer rounded-sm border border-[var(--shell-input-border)] bg-[var(--shell-input-bg)] px-4 text-[13px] text-[var(--shell-content-text)]" onClick={() => setEditing(null)}>{s.cancel}</button>
      </div>
    </div>}
  </div>
}
