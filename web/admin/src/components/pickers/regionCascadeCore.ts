...
// 契约（W-0904-UI U1 增强）：GET /geo/subdivisions 支持 countryCode/parentCode/keyword/limit（默认50最大200），
// 行含 code/译名 name/level/hasChildren，兼容增强字段 parentCode/countryCode；
// GET /geo/default-country 返回默认国家 alpha2，空值/失败一律兜底 PH。
...
/** 区划列表：按契约拼查询参数（limit 默认 200），行字段归一化（name 缺省回退 displayName）。 */
export async function fetchSubdivisions(query: SubdivQuery, fetchImpl: FetchLike = apiFetch): Promise<SubdivRow[]> {
  ...
  if (query.countryCode) q.countryCode = query.countryCode
  if (query.parentCode) q.parentCode = query.parentCode
  if (query.keyword) q.keyword = query.keyword
  const rows = (await fetchImpl<Array<Record<string, unknown>>>('/geo/subdivisions', { query: q })) ?? []
...
/** 默认国家：兼容 {alpha2} 对象与裸字符串；空值/解析失败/请求失败一律兜底 PH（契约 N3）。 */
 ...
 *  行无 parentCode 增强字段时退化为单节点链（不拉全量），由 UI 展示并告警。 */
