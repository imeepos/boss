...
export function VerticalBars({
  ...
  return (
    <div className="flex h-40 items-end gap-3 overflow-x-auto py-5">
      {labels.map((lab, i) => (
        <div key={lab + i} className="flex h-full min-w-12 flex-col items-center justify-end gap-2" title={`${lab}: ${values[i]}`}>
          <div className="text-xs font-medium text-muted-foreground">{values[i]}</div>
