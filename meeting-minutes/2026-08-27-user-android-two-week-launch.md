...
# 第三部分（同日执行推进跟踪，主持人实时过账）

> 以下为会议后执行推进中新增的裁决、缺陷与进展，随执行实时追加；台账权威读本为 docs/ops/user-android-launch-status.md。

## 十一、执行推进关键裁决

| 日期 | 裁决 | 内容 | 状态 |
|---|---|---|---|
| 08-27 | 用户拍板 | 首发包含**公网/异地用户** → https 公网域名提级为首发硬门槛 | ✅ adopted note 2026-08-27-user-android-https-hard-gate.md |
| 08-27 | 阿澈裁定 BUG-01 | orders tab label 依 nav.js 契约改「订单」，路由保持 Route.Orders（spec L7 描述过期交产品修文档债） | ✅ 合入 e8f827fb，真机回归关闭 |
| 08-27 | 阿澈裁定 BUG-02 | 根因实锤：后端 HTTP 401=真凭证失效（auth middleware），40100=业务校验失败（验证码错/旧口令错，HTTP 200+信封）；客户端 isUnauthorized 一刀切清 token 误报放大。修正：仅 HTTP 401 清除 + 短窗口冷却单播 + 40100 细化文案 + 定位日志 | ⏳ 分支 fix/unauthorized-40100-cleanup 待合入 |
| 08-27 | 阿澈裁定 OB-06 | 登录未勾协议点登录补提示「请先阅读并同意《用户协议》」（enabled 恒 true onClick 判断），保留灰底视觉 | ⏳ 同 BUG-02 分支 |
| 08-27 | 木子裁决推送缺口 | 系统通知栏推送 = **P2 首发后可补，不阻断上线**（核心闭环完备、敏感权限首发弹窗伤体验、push_devices 已落库为干净底账） | ✅ |
| 08-27 | 木子裁决 4b 风险 | ①注册验收=真链路（真实新号开户→注册→建号→登录，禁人工造数账号）；②风控调 A 档 20/10 保持 enabled=true+审计；③首发前按 ≥10 倍缓冲做预热预案（开关化） | ✅ 明远执行（台账 §十四） |

## 十二、执行推进中的新事实（明远实测）

- **102 端点复测**：/push/device 与 /customer-registrations 的 404 系首测用 GET 探测仅定义 POST 的端点所致（方法不存在≠路由缺失），**B 轨「404 漂移」为勘察方法学误报，已 Amended 更正**；空 body 复测返回 42200 未落库，无造数污染。
- **压测/告警（60VU）**：user 端全 P95 达标（接口层最差订单列表 86.6ms；登录真码全链路 2792ms 受冷却制约）；5 条 user 关键路径告警规则已入 prometheus 生效。
- **https 后端动作（§十三）**：独立 nginx 443→28080（https://192.168.0.102 全端点可达）；正式发布记录 id=7 versionCode=4 PUBLISHED 注入（灰度字段已存在）；https 通道 30VU 压测 P95=99.7ms 0 失败。
- **风控调档**：risk.direct.phoneCap 5→20、addressCap→10（走审计），291 地址真实下单验证新阈值生效；造数清理完成。

## 十三、缺陷台账（真机实锤，佳宁）

| 编号 | 级别 | 内容 | 状态 |
|---|---|---|---|
| BUG-01 | P1 | 底部「账单」tab 路由错绑（应「订单」，Route.Orders 语义） | ✅ 修复合入 e8f827fb，真机回归关闭 |
| BUG-02 | P1 | 登录态偶发闪断：40100 业务码被当凭证失效全局清 token | ⏳ 修正分支待合入（01c067fa + OB-06 81cd1167） |
| OB-02 | 观察 | 登录页「获取验证码」tap 疑似双触发（42300 但库已生成码） | ⏳ 分析中：注入环境 tap 双事件概率大，复现法已定（logcat 请求计数对照） |
| OB-03~05 | 观察 | 地址表单空提交无校验提示 / 缴费页空数据文案 / input 注入丢字（测试方法限制） | 已登记 |
| OB-06 | 观察 | 登录未勾协议点登录静默无响应 | ✅ 补提示（随 BUG-02 分支） |

## 十四、协调事项（并行会话治理）

- 真机 ee9999eb 被并行会话反复覆盖安装 → 基线不稳；对策：lastUpdateTime 核对 + 紧凑循环 + 转 emulator-5554（API 35）独立基线（佳宁执行中）。
- 佳宁 commit 意外包含明远 scripts/load/ 压测脚本（内容无损，已确认归属）。
- 遗留：注册/密码登录/地址保存/订单详情/支付真链路待真机窗口恢复或模拟器执行。