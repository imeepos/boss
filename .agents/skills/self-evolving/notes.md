...
### 本轮任务反思（2026-08-24）
- 哪个坑浪费了最多时间？Dashboard 日期 flake 初修只移动相对时间，仍受 UTC/业务时区和跨日窗口影响；真正稳定的修法是显式固定 `clock` 业务时区，并在测试结束恢复全局时钟状态。
- 这个 skill 有没有提前警告我？有：后端知识索引提示日期/时区要先实测，lessons 也要求时区与 `clock` 口径一致；但我第一次没有把测试 fixture 和全局业务时区一起固定，先走了不充分的相对时间修补。
- 重来一次我会怎么做？先读 dashboard 的 `clock.Now`/`trendWindow`/`trendBucket`，在测试入口 `clock.Set("Asia/Manila")` + `t.Cleanup` 恢复 UTC，再设计跨日/跨周 fixture；完成后先单测 `-count=10`，再跑全量门禁。另：部署验证不能只看 healthz，本轮已补看容器启动时间及 ETL 路由日志，但因远端镜像无 revision label、扫描周期尚未自然触发，历史 6 条 OPEN 派单清理未取得实机数据证据，需后续用 admin API/远端 PG 查询闭环。
