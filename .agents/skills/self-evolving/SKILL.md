...
14. **【已犯 7 次】run_code 里工具调用发车前必须自检必填参数成对齐全** —— edit 缺 new_string 连发 4 次(quadfixa 轮);2026-09-05 负责人轮内层 tools.bash 漏必填 description 再连炸 3 次,报错不指明哪层调用极易误判;发车前逐项默念每个内层调用的必填键(edit: old+new;bash: command+description 必填)。变体(2026-09-06 负责人轮 3 次):对象字面量键名手滑多一个引号(description": / new_string":)整程序 parse error 全段不执行——键名默念也算自检的一部分。
