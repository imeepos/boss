...
| 模型不支持图像输入却尝试读图分析 | 5 | 2026-08-18(使用 read_image 读取截图失败), 2026-10(本次 GLM-5.3-Flash 读取截图失败), 2026-09-01(worker 页截图后再犯,GLM-5.3-Flash 不声明图像输入被拒;改用 cdp --eval DOM 断言为证据), 2026-09-04(会话清理验收读图被拒,直接转持久化登记表机械验证), 2026-09-05(W-0907 负责人验收读图被拒,转 verify-deploy --feature + 截图尺寸佐证) | 浪费时间尝试不支持的功能,需先检查模型能力,改用 DOM/计算样式断言 |
