...
| ssh+psql/嵌套 bash 执行 SQL 时叠引号,外层 bash 吞掉 → column does not exist / syntax error / 查询静默返空 | 6 | 2026-08-26(清理 PAY-12 双引号被当标识符), 2026-08-26(checkout 探针内联 bash -c 双引号全炸), 2026-08-26(webhook 验收脚本 db() 用 -c 包 SQL,查询返空误判'未落账',实为引号被吞); 2026-08-27(构造分叉单 ssh+heredoc+反斜杠转义单引号失败 3 次才换 scp+<<'SQL'); 2026-08-27(查询用 `psql -c \"...'...'...'...\"` 外双内单也炸); 2026-09-01(entity-staff 验收清理用 `psql -tAc` 再接 heredoc,heredoc 走 stdin 而 -c 吃不到参数报 "option requires an argument",清零断言白跑一轮) | 一律 `psql ... <<'SQL'` 单引号 heredoc 传 stdin(不带 -c);SQL 字面量单引号,绝不在 -c 字符串里叠双引号;查询返空先手工 psql 复核再下'功能坏'结论;复杂多行 SQL 先 scp 到远端 /tmp 再 `docker exec ... psql -f`,避免嵌套引号地狱 |
...
| run_code 程序字符串里出现反引号或 ${,被宿主模板包装层解析破坏 | 3 | 2026-09-01(envelope.ts 编辑 old_string 含模板字符串,两次 parse error); 2026-09-02(Go 源码经模板字符串 write,Go 字面量 \n 被吃成真实换行,resp.go 整文件 string literal not terminated) | 含特殊字符的 old_string 用 String.fromCharCode(96)+拼接构造,或选无反引号锚点;**传 Go/模板类源码一律行数组 L.push()+join 组装**,源码内的转义序列(\\n/\\t)双写;已登 SKILL.md 红线#11 |
