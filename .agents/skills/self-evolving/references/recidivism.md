...
| edit 前不用 read 工具看文件 / 凭上轮记忆拼 old_string | 17 | 2026-08-18(cat 后 edit 被拒), 2026-08-18(sed 后 edit 被拒), 2026-08-18(UserMenu old_string not found) | 每次浪费一轮重试 |; 2026-08-20(并行会话改文件,edit 连续 file changed since read); 2026-08-21(data-relations.md 用 bash sed 读后 4 个 edit 全被拒,重读一轮); 2026-08-25(notify_test.go python 改后 edit 拒,locale 凭记忆 edit 拒); 2026-08-24(etl_pg.go sed 读后 edit 拒); 2026-08-27(同一文件主树读过、worktree 副本未按 worktree 绝对路径 read,handler 与 admin.yaml 两个 edit 连拒;recidivism.md 本身 bash tail 后 edit 也被拒——read 状态按绝对路径跟踪,bash tail/head 不算已观察,换树/换文件必用 read 工具重读); 2026-09-01(worktree 会话 8 连击总爆发:provision.go/en-US/ms-MY/product types/provision types/template index/customer.yaml/pg_binding_test 全部因「只读过主树副本或 bash cat/sed 输出」被拒,连更新台账本身都先中一枪——worktree 轮开工先对将 edit 的每个文件 read worktree 绝对路径,一轮省 8 次重试); 2026-09-03(gofmt -w 重写 strict_test.go 后凭旧 read 直接 edit 被拒——同会话内任何工具改写文件后 edit 前也要重读) |
...
| run_code 程序字符串里出现反引号或 ${,被宿主模板包装层解析破坏 | 6 | 2026-09-01(envelope.ts 编辑 old_string 含模板字符串,两次 parse error); 2026-09-02(Go 源码经模板字符串 write,Go 字面量 \n 被吃成真实换行,resp.go 整文件 string literal not terminated); 2026-09-02(JS 双引号串内 JSON 转义换行变真实换行截断程序体;拼接辅助符被写成字面量致 Expected ident,均 parse error) | 含特殊字符的 old_string 用 String.fromCharCode(96)+拼接构造,或选无反引号锚点;**传 Go/模板类源码一律行数组 L.push()+join 组装**,源码内的转义序列(\\n/\\t)双写;已登 SKILL.md 红线#11;2026-09-04 三变体:bash 命令串整体也被宿主模板化(远端串内 美元符花括号 直接 ReferenceError)、python heredoc 内反斜杠转义字面量被宿主解析成真实换行截断程序、token 重放 294 行 bash 脚本时 JSON body 转义形态写错(应以 "\{ 开头写成 "\"{)致造数 body 双重编码 422,直到完整 E2E 才暴露——含美元符花括号与反斜杠的长内容统一 token 法(占位符+fromCharCode 还原)一次成型,生成后必须对关键行 printf 原样对照,造数 API 在完整 E2E 前先单点冒烟 |
