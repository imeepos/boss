...
| 坑 | 次数 | 发生日期 | 后果 |
|---|---|---|---|
| bash 默认 cwd 是主树而非 worktree,python/sed 批量编辑跑错树 | 2 | 2026-08-22(改了主树 check-contract-sync/routes.go;另一次 FileNotFound 才发现); 2026-08-26(调研文档写进主树内嵌套目录 boss/boss-wt-addr-research/,git commit 静默落 main,靠输出标记 `[main 71bb240e]` 发现) | worktree 内每条 bash 显式带 workdir,heredoc 开头 pwd 自检;`worktree add ../name` 建在仓库外侧,写文件用 `git worktree list` 核对绝对路径,commit 输出方括号看分支名 |
