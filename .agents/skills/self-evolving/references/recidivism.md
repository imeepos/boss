...
| 并行会话推 commit 后,我方 worktree ff-merge 失败,误以为"合并失败=工作丢失" | 1 | 2026-08-22(A2 丢失的直接诱因) | ff-merge 失败 ≠ commit 丢失;先看冲突类型(diverging→rebase;concrete conflict→manual);commit 在 worktree 内安全,本地 reflog/refs/heads/<branch> 仍可访问 |
