...
| ssh+psql/嵌套 bash 执行 SQL 时叠引号,外层 bash 吞掉 → column does not exist / syntax error / 查询静默返空 | 10 | 2026-08-26(清理 PAY-12 双引号被当标识符), 2026-08-26(checkout 探针内联 bash -c 双引号全炸), 2026-08-26(webhook 验收脚本 db() 用 -c 包 SQL,查询返空误判'未落账',实为引号被吞); 2026-08-27(构造分叉单 ssh+heredoc+反斜杠转义单引号失败 3 次才换 scp+<<'SQL'); 2026-08-27(查询用 `psql -c \"...'...'...'...\"` 外双内单也炸); 2026-09-01(entity-staff 验收清理用 `psql -tAc` 再接 heredoc,heredoc 走 stdin 而 -c 吃不到参数报 "option requires an argument",清零断言白跑一轮); 2026-09-02(oltsim 回归轮 4 连:双美元号被 PID 展开×2、IN (PENDING,DOING) 枚举未引号返空×2) | 一律 `psql ... <<'SQL'` 单引号 heredoc 传 stdin(不带 -c);SQL 字面量单引号,绝不在 -c 字符串里叠双引号;查询返空先手工 psql 复核再下'功能坏'结论;复杂多行 SQL 先 scp 到远端 /tmp 再 `docker exec ... psql -f`,避免嵌套引号地狱 |
