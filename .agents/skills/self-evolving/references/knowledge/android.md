...
| 25 | lessons（2026-08-27） | 全新 worktree 首次构建前先复制 gitignored 的 `mobile/user/android/local.properties`(含 sdk.dir),否则 gradle 找不到 SDK;密钥/证书资产放主树 gitignored 位置而不是 worktree(worktree remove 物理删目录会带走 keystore) |
| 26 | lessons（2026-08-27） | instrumented 测试断言权限声明:`getPackageInfo(GET_PERMISSIONS).requestedPermissions` 当前 API 可空,须 `?.contains(...) == true` |
| 27 | techniques（2026-08-27） | 本机 AVD `test_device` 无头跑 connected 测试:`emulator -avd test_device -no-window -no-audio -no-boot-anim -gpu swiftshader_indirect`,轮询 `getprop sys.boot_completed=1` 再 `connectedDebugAndroidTest`,收尾 `adb emu kill` |
| 28 | known-issues（2026-09-26 实测 AGP 8.13.1） | `connectedDebugAndroidTest` 不认 Gradle 的 `--tests` 选项(报 `Unknown command-line option '--tests'` 秒失败,那是 JVM Test 任务的选项);过滤 connected 测试类用 `-Pandroid.testInstrumentationRunnerArguments.class=完整类名`(等价 am instrument -e class),日志看 "Starting N tests on <设备>" 核对只跑了本类 |
| 29 | lessons（2026-09-26） | Compose 测试断言:`assertDoesNotExist()`/`assertExists()` 是 SemanticsNodeInteraction 的**成员函数**,不用 import;`assertIsDisplayed()`/`performClick()` 才是扩展函数,漏/多 import 都会 `Unresolved reference`(写测试前对照既有测试的 import 列表照抄最稳) |
| 30 | known-issues（2026-08-29 实测） | `androidx.compose.ui:ui-test-manifest` 必须 **debugImplementation** 不能 androidTestImplementation:后者把占位 ComponentActivity 合进 `*.test` 包,启动即抛 `Intent in process X resolved to different process X.test`(100% 复现非并发竞态);排查钥匙=拿"参照端同设备跑通"作基线,逐行 diff 两端 build.gradle.kts 的依赖 configuration |
