...
`Submit` 在派生归属法人后、发单号前调用 `FindTemplateForOffer(offerID, 法人)`，
不可解析即拒单（40900 + 原因透传）：不可开通 = 不可售，杜绝"钱付了卡环节 7"。
机制性解决存量无绑定无带宽套餐的处置问题——未绑定 = 不可订购，无需事后卡单或数据订正。

覆盖范围：直营与伙伴单**天然同覆盖**——`submitPartnerAtomic` 清零法人后委托
`submitRegular`，预检在委托路径内随事务执行（TMFC002 资格校验本就不分捕获通道）。
增值类套餐（无带宽、无绑定）据此维持**不可售**状态：给 IPTV 等绑定 OLT 模板会让
无意义开通变"成功"，语义更糟；待 IPTV 独立开通通道立项后再谈可售化。
