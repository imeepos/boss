...
## P2-W1+W2 验收结论(2026-09-06,Lead 独立复跑)
并行波交付 13 表 CRUD+操作矩阵(fingerprint de5e3400 已上线)。Lead 不采信自报,
独立复跑三个机械验收脚本全部 exit 0:E2E-AMS-TABLES-OK / E2E-ASSET-CRUD-OK /
E2E-ASSET-LINKAGE-40断言(P1 回归无损伤);make check 绿。**验收通过**。
P2-W3(CRUD 矩阵推广至组织/客户/订单/计费):**暂缓**——先经真实用户对已交付
13 表做一轮使用验收,再按需求热度排期;不自动扩张范围。
遗留知会:ScrapDialog 三要素升级(P3);tag_events 历史绑定无事件(可补录);
AMS-E2E 前缀字典数据按设计留存可审计。

## Lead 裁定(2026-09-06):报废确认统一为三要素
P2-W1 的 ScrapDialog(资产页)与 P2-C 的三要素弹层(标签页「报废绑定资产」)并存。
裁定:统一采用三要素口径——影响面清单 + 不可逆/恢复路径说明 + 红色确认键默认禁用
且原因必填(报废需精确输入资产编码)。后续波次把 ScrapDialog 升级对齐(小改动,
列 P3 待办);过渡期两套确认并存不视为缺陷(均落审计与事件)。另:P2-C 实现已随
ed3335a4 合入 main 并上线(102 冒烟通过),其分支 feat/tag-events-consumer @ 103285f1
按协议清理即可。
