...
- {{ 顶部品牌头部：如 深蓝 BrandBlue700 线性渐变（135°，#273F70→#3A5A9C），底部左右大圆角 24dp，背景向上延伸衬到状态栏后方（statusBar 染色），标题白字；明确"不用 TopAppBar 默认灰底" }}
- {{ 底部导航栏：如 白底 + 顶部 1dp 描边、无阴影、无胶囊指示器，选中态仅图标变主色 + 文字加粗，4 个 Tab 图标 + 文字，高 64dp；明确"不用 NavigationBar 默认容器色/指示器" }}
- {{ 其它异形/渐变/悬浮元素：如 进行中订单卡向上叠在头部上，marginTop -24dp }}
