...
### 4.1 RN 族第二色板裁决：维持第二主题（2026-08-28 定案）

- 现状：RN 登录族存在独立第二色板，有 v2 设计稿背书且已真机验收，不能简单抹掉
- 处置（2026-08-28 定案）：**已定案维持第二主题**（RnPalette.kt，冻结新增不变）——依据 designs/login-register-states-v2.spec.md:16 色值规范，RnPalette 与设计稿逐色一致；并入全局须以"设计出 v3 更换登录族主蓝"为前提，届时改 RnPalette 18 常量值并重走登录/实名真机验收
