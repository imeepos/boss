...
|---|---|---|---|---|---|
| 1 | 经营区域树（大区/省/城，独立于地理地址树） | A01/A02/A04 | 基础平台 | `regions` 表 + 与 `addresses` 解耦 | ✅ `region.html` |
| 2 | 子公司/法人实体（品牌隔离） | A03/A04/A08 | 基础平台 | `legal_entities` 表 | ✅ `company.html` |
| 3 | 部门实体 | A05/A06 | 基础平台 | `departments` 表 | ✅ `department.html` |
| 4 | 岗位实体 + 岗位↔角色分离 | A07/A08 | 基础平台 | `posts` 表 + `post_roles` 关联 | ✅ `post.html` |
| 5 | 账号组织归属（子公司/部门/岗位/区域） | A03/A05/A09 | 基础平台 | `accounts` 增 `legal_entity_id/dept_id/post_id/region_scope` | ✅ `account.html` 补列 |
| 6 | 数据权限（数据范围，功能权限之外的约束） | A01~A09 | 基础平台 | `data_scopes` 表 + JWT 增数据范围声明 | ✅ `datascope.html` + `HasDataScope` |
| 7 | 跨区域调配审批入口 | A02 | 网络资源 | `transfer` 调配菜单（或并入 reserve） | ✅ `transfer.html` |
| 8 | 品牌×区域交叉汇总 | A04 | 经营分析 | `analytics` 增「子公司/品牌」维度 | ✅ `analytics.html` 补维度 |
| 9 | 调度工作台（调度岗与师傅岗分离） | A07 | 订单与工单 | `dispatch` 增岗位视图 | ✅ `dispatch.html` 补工作台 |
