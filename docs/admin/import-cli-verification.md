...
| 7 | 客户 FK 不存在 | POST /customers legalEntityId=99999 | `42200` 参数非法 ✅（2def20f7 部署后复验） |
...
2. **客户 FK 不存在 → 500**：`customer.ErrForeignKeyViolation` 未注册 httpx 映射。修复 `2def20f7`（含岗位模板非法码 `POST-IMP-01`→`post_imp_01`），部署后 CLI 复验为 `42200`。
...
批量导入 12 个页面对应的创建端点在 CLI 直连下全部通过：重复一律 409、参数非法一律 422、未认证一律 401；任务契约（total/skipped 持久化+筛选）验证通过；客户 FK 映射已部署并复验为 42200。

注意：本轮追加的前端权限门禁、401 刷新、未知 kind 展示和地址/Geo 任务登记错误处理尚未部署，不能将其写入本表的“已验证”结论。
