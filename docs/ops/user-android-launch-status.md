...
| 无 ANR/复现崩溃 | ✅ | 14 轮 connected(11/11)+ 真链路 UI 驱动持续无 FATAL/ANR(观测记录入 checklist) |
| 压测 P95<1.5s + 监控告警 | ⏳ 后端 | 102 D-3 压测为明远职责,本仓库无可执行项 |
| 关键路径 connected 全绿 | ✅ | connected 11/11 全绿零跳过;E2E 8 读调用(登录真码→产品→实名→订单→账单→套餐/地址/券) |
...
| 关键路径 connected 全绿+6 条回归 | ✅ | 11/11(自动化)+ 走查量化四 tab+子页;下单→支付 destruct 腿列 P2(待回收通道) |
