...
| 压测 P95<1.5s + 监控告警 | ⏳ 后端压测已出报告 / ⏳ user 端专项待跑 | 管理端基线 P95<500ms(10-20VU)已达标见 load-test-report.md；user 端登录/下单/支付专项压测+监控告警为明远 D-3 职责 |
...
4. 102 压测 P95<1.5s + 监控告警——明远。管理端基线已达标（load-test-report.md）；**user 端专项（登录/下单/支付 60VU 档）待 D-3 前跑 + 监控告警接 5xx/超时**。第一轮 k6 报告见 `scripts/load/api-load.js`。
