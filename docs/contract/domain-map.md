...
| C | internal 包（12 域） | 当前 Go 骨架 `internal/domain/*` | 代码包 | 落地位置 |
| D | admin 页面分组（16 组 46 页） | `docs/admin/menu.js` | UI 分组 | 前端落地 |

> 关键事实：A/B 是"菲律宾 BOSS"权威体系（21 域），C/D 是"中国 9 阶段"视角的子集（12 域/16 组）。
...
| 经营分析 BI | BI | AG-12 | `analytics` | 阶段9 | intel | analytics/report |
...
menu.js 共 13 分组 45 菜单页（另 `login.html` 为登录散页，不进菜单）。表列"页面"为该分组全部 href。
...
| bss 客户与资费 | CRM + PROD | `customer` | 阶段2 | customer/product |
...
| boss 订单与工单 | ORD + CS | `order` | 阶段5 | order/dispatch/dismantle/complaint/callback |
