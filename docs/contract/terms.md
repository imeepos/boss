...
| 施工整改 construction_defects.status | OPEN / RECTIFYING / VERIFIED | 待整改 / 整改中 / 复验通过关闭；OPEN 可直达 VERIFIED（现场即改即验）；VERIFIED 终态。**验收前置：项目无未 VERIFIED 整改项**（AcceptProject 事务内校验）。迁移 000214（P0-C） |
| 施工测试 construction_tests.result / test_kind | PASS / FAIL；OTDR / OPTICAL_POWER / CONNECTIVITY | 资源级测试证据（设施/光缆段/纤芯/端口），append-only 每次读数即事实；验收证据链组成。迁移 000214（P0-C） |
...
| ODN 资源链状态 odn_resource_chain | lifecycle_status 同设施四态;port_status IDLE/RESERVED/USED/DISABLED;laying_method AERIAL/UNDERGROUND/SUBMARINE/MICROTRENCH/INDOOR;row_status NOT_STARTED/PENDING/APPROVED/EXPIRED/NA;pece_status PENDING_SIGN/SIGNED/STAMPED/NA;分光比 1:2/1:4/1:8/1:16/1:32/1:64/1:128;网格归属 prv_code/city_prefix/grid_code 三列一体(可选,000222,W3 收口 F2) | 迁移 000210(W3);模板中文标签映射:资源状态 规划→PLANNED/已安装·已测试→IN_BUILD/在用→IN_SERVICE/已报废→RETIRED/留空→PLANNED(绝不当作已安装/在网);总分光比=一级×二级;带归属链行导入联动备案网格+MH 锚点设施(裁定 adopted 2026-09-07-infra-w3c-import-linkage);fields.md 1.5.12 |
