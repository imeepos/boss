...
> 版本 V1.0（2026-08-17 首发）｜权威源：`terms.md` / `fields.md` / `domain-map.md` / migrations / Go 实体
> 定位：把「数据建模与 admin/user/worker 三端页面」的对齐状态固化为可销项清单。
> 规则：字段/状态/术语以 `terms.md` 为准；页面列名 ↔ 字段名 ↔ 枚举以 `fields.md` 为准；已销项标 ✅ 并注明提交。
>
> **V1.x 修订历史（仅追加不覆盖 V1.0 销项）**：
> - §7.2 V1.1（2026-08-19）补 OpenAPI 三端 56 路由 + 9.2 信封三方对齐
> - §10 V1.2（2026-08-20）ODN 地理空间编码规范对齐（E4-E11）
> - §11 V1.3（2026-08-21）表结构对账 000061-089（E12-E16）
> - §0 顶部新增本条说明（2026-08-27）：V1.0 引用「68 表」「37 文件」「enums.ts」均为首版基线快照，
>   后续新增表/路由/枚举不在本台账追溯范围，按各子节版本头为准。
...
> 故字段名与 Go 实体（68 表关系模型）存在**视图导向命名差**，属设计使然、非漂移。
