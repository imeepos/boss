...
func (s *server) portal(name string) (*portalCfg, error) {
	p, ok := s.Portals[name]
	if !ok {
		return nil, fmt.Errorf("未知端 %q(可用 user/worker)", name)
	}
	return p, nil
}

// newServer 从 getenv 装配两端客户端(注入便于测试)。
...
// resolveKey 端专属 key 优先,BOSS_API_KEY 兜底;返回实际采用(或应配置)的环境变量名。
func resolveKey(getenv func(string) string, portal string) (key, envName string) {
	specific := map[string]string{"user": "BOSS_USER_API_KEY", "worker": "BOSS_WORKER_API_KEY"}[portal]
...
func main() {
	...
	logf("version=%s server=%s user_key=%s worker_key=%s",
		s.Version,
		s.Portals["user"].Client.Server,
		keyState(s.Portals["user"]), keyState(s.Portals["worker"]))
